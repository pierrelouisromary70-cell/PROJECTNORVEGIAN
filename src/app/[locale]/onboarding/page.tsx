...
export default function OnboardingPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  async function finish() {
    ...
    await supabase.from('profiles').upsert({
      ...
      has_done_intervals: hasIntervals,
      goal,
      vdot: vdot || 40,
    ...
    await supabase.from('race_results').insert({
      user_id: user.id,
      distance_meters: RACE_PRESETS[raceDist],
    ...
    <main className="min-h-screen grid place-items-center px-6 py-10">
      <div className="card w-full max-w-xl">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-fjord-950">{t('title')}</h1>
            ...
            <div>
              ...
              <div className="flex gap-2">
                {(['male', 'female', 'other'] as const).map((s) => (
                  <button key={s} type="button" onClick={() => setSex(s)} className={`btn ${sex === s ? 'bg-fjord-600 text-white' : 'bg-fjord-100 text-fjord-800'}`}>
        ...
        {step === 2 && (
          <section className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setHasIntervals(true)} className={`btn ${hasIntervals ? 'bg-fjord-600 text-white' : 'bg-fjord-100 text-fjord-800'}`}>Oui</button>
                <button type="button" onClick={() => setHasIntervals(false)} className={`btn ${!hasIntervals ? 'bg-fjord-600 text-white' : 'bg-fjord-100 text-fjord-800'}`}>Non</button>
              </div>
            </div>
            <p className="text-sm text-fjord-600">Niveau détecté : <strong>{level}</strong></p>
          </section>
        )}
        ...
        {step === 3 && (
          <section className="space-y-4">
            ...
            <button type="button" onClick={() => setGoal('progress')} className={`w-full text-left p-4 rounded-xl border ${goal === 'progress' ? 'border-fjord-600 bg-fjord-50' : 'border-fjord-200'}`}>
              <div className="font-medium text-fjord-900">{t('goalProgress')}</div>
              <div className="text-sm text-fjord-700">Plus de plaisir, moins de pression. Volume modéré, intensité contrôlée.</div>
            </button>
            <button type="button" onClick={() => setGoal('perform')} className={`w-full text-left p-4 rounded-xl border ${goal === 'perform' ? 'border-fjord-600 bg-fjord-50' : 'border-fjord-200'}`}>
              <div className="font-medium text-fjord-900">{t('goalPerform')}</div>
              <div className="text-sm text-fjord-700">Performance maximale. Double seuil, gros volume, exigence.</div>
            </button>
          </section>
        )}
        ...
        {step === 4 && (
          <section className="space-y-4">
            ...
            <p className="text-sm text-fjord-700">Renseignez une performance récente. On en déduit votre VDOT et toutes vos allures.</p>
            <div>
            ...
            <div className="rounded-xl bg-fjord-50 p-4">
        ...
        {step === 5 && (
          <section className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-fjord-600 mt-1">Laissez vide si pas de contrainte particulière.</p>
            </div>
          </section>
        )}
            ...
            {sex === 'female' ? (
              <div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setTrackCycle(true)} className={`btn ${trackCycle ? 'bg-fjord-600 text-white' : 'bg-fjord-100 text-fjord-800'}`}>Activer</button>
                    <button type="button" onClick={() => setTrackCycle(false)} className={`btn ${!trackCycle ? 'bg-fjord-600 text-white' : 'bg-fjord-100 text-fjord-800'}`}>Plus tard</button>
                  </div>
                </div>
              </div>
            ) : (
              <p className="text-sm text-fjord-700">Vous êtes prêt. Cliquez sur Continuer pour générer votre premier plan.</p>
            )}
          </section>
        )}
